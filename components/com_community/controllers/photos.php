...
// no direct access
    ...
    class CommunityPhotosController extends CommunityBaseController
    {
        ...
        public function ajaxConfirmRemovePhoto($photoId, $action = '', $updatePlayList = 1)
        {
            ...
            if (!$handler->hasPermission($album->id) && ($photo->creator != $my->id)) {
                $json = array(
        ...
        public function ajaxCreateAlbum($albumName, $id, $context, $location = '', $description = '', $privacy = '')
        {
            ...
            JRequest::setVar('name', $albumName);
            JRequest::setVar('location', $location);
            ...
            if ($context == 'group') {
                ...
                $event->load($id);
                JRequest::setVar('eventid', $event->id);
            } elseif ($privacy !== '') {
                JRequest::setVar('permissions', $privacy);
            }
        ...
        public function editAlbum()
        {
            ...
            if ($jinput->getMethod() == 'POST') {
                ...
                $referrer = $jinput->post->get(
                    ...
                    'myphotos',
                    'STRING'
                ); //JRequest::getVar('referrer', 'myphotos', 'POST');
...
//			$url = $handler->getEditedAlbumURL( $albumid );
        ...
        public function removealbum($albumId = 0)
        {
            ...
            // Get the album id.
            $my = CFactory::getUser();
            $id = JRequest::getInt('albumid', $albumId);
        ...
        private function _checkUploadedFile($imageFile, $album, $handler, $commentImage = false)
        {
            ...
            $isDefaultPhoto = $jinput->request->get(
                ...
                false,
                'NONE'
            ); //JRequest::getVar('defaultphoto', false, 'REQUEST');

            if ($album->id == 0 || (($my->id != $album->creator) && $album->type != PHOTOS_GROUP_TYPE && $album->type != PHOTOS_EVENT_TYPE)) {
            ...
            if ($handler->isExceedUploadLimit(false) && !COwnerHelper::isCommunityAdmin()) {
                ...
                $eventId = JRequest::getInt('eventid', $album->eventid, 'REQUEST');
                $config = CFactory::getConfig();
...
// 			            $image->rotateImage($public, 90);
    ...
    class CommunityControllerPhotoGroupHandler extends CommunityControllerPhotoHandler
    {
        ...
        public function __construct($groupid)
        {
            $mainframe = JFactory::getApplication();
            $jinput = $mainframe->input;
            $this->eventid = $jinput->get('groupid', $groupid, 'INT');
            parent::__construct();
        }
